...
namespace BFConfigApp
{
    public partial class ConfigForm : Form
    {
        ...
        int TotalNumCases = 0;
        public ConfigForm(MainScreen form)
        {
        ...
        private void nextBtn_Click(object sender, EventArgs e)
        {
            ...
            {
                caseFairConfig = new CaseFairConfig(this);
            }
        ...
        private void SaveSession()
        {
            try
            {
                ...
                SessionManager.CurrentCaseSession.AllCaseNum["SmallSilver"] = int.Parse(smallSilverTB.Text);
                SessionManager.CurrentCaseSession.AllCaseNum["SmallRainbow"] = int.Parse(smallRainbowTB.Text);
                SessionManager.CurrentCaseSession.AllCaseNum["Medium"] = int.Parse(mediumTB.Text);
                SessionManager.CurrentCaseSession.AllCaseNum["PebbleTop"] = int.Parse(mediumPebbleTB.Text);
                SessionManager.CurrentCaseSession.AllCaseNum["Large"] = int.Parse(largeTB.Text);

                if (tableTopFairCheckBox.Checked) //checks if the fair is table top or not
                {
                    SessionManager.CurrentCaseSession.IsTableTop = true;
                }
                else
                {
                    SessionManager.CurrentCaseSession.IsTableTop = false;
                }

                SessionManager.CurrentCaseSession.Linens = numLinenComboBox.SelectedItem?.ToString() ?? "Not selected"; //linens are saved

                SessionManager.CurrentCaseSession.MaxCashRange = int.Parse(maxRangeTB.Text);//min and max range are saved
                SessionManager.CurrentCaseSession.MinCashRange = int.Parse(minRangeTB.Text);

                SessionManager.CurrentCaseSession.TotalCaseNum = TotalNumCases;
